...
if __name__ == '__main__':#ignore this it's here for testing reasons
    ...
    #the second word in each of the peices of yoru scema touple is the data type heald by that feild


    table = [#this is a tuple, tooples are immutable and can be passed as an argument in sqlite to create or edit a table
        #I'm using an example of video games for thie databese but you will edit this toople to fit your particular databse
    ...
    cursor.executemany(f"insert into {table_name} values (?,?,?)", table)#the number of question marks here should corrispond to the number of elements in your table
    #saves your database
    connection.commit()
